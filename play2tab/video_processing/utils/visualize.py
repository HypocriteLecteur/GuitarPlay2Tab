...
def draw_fretboard(image: cv2.typing.MatLike, fretboard: Fretboard) -> None:
    ...
    draw_houghline_batch(image, fretboard.frets)
    ...
    if pnts1.shape[0] == 2:
        pnts1 = pnts1[0, :] + (np.arange(0, 6)/6).reshape((6, 1)) @ (pnts1[1, :]-pnts1[0, :]).reshape((1, 2))
        pnts2 = pnts2[0, :] + (np.arange(0, 6)/6).reshape((6, 1)) @ (pnts2[1, :]-pnts2[0, :]).reshape((1, 2))
