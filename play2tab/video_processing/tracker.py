...
class Tracker(TrackerInterface):
    ...
    def strings_locating(self, frets, gray, strings_num=6, is_visualize=False) -> Tuple[NDArray, NDArray, NDArray]:
        ...
        # for i in range(fret_dists.size):
            # cv2.circle(template_res_cdst, (offset_fret_dists[i], strings_start_y[i]), 3, (0, 0, 255))
            # cv2.circle(template_res_cdst, (offset_fret_dists[i], strings_start_y[i]+int(peak_spacings_fit[i]*(strings_num-1))), 3, (0, 0, 255))

        # template_res_cdst = cv2.cvtColor(template_res, cv2.COLOR_GRAY2BGR)
        # draw_houghline_batch(gray, upper_line)
        # draw_houghline_batch(gray, lower_line)
    ...
    def track(self, frame: MatLike, fgmask: MatLike, fretboard: Tuple) -> Tuple[bool, Union[Fretboard, None]]:
        ...
        homography = utils.find_projective_rectification(lines, gray.shape)
