...
class ColoredSlider(QSlider):
    ...
    def _position_to_slider_x(self, position):
        ...
        # return int(slider_pos * (self.width() - 2)) + 1
...
class ImageWindow(QWidget):
    ...
    #     print(new_width)
        ...
        # Resize the pixmap to fit the window's new size while maintaining aspect ratio
...
class VideoPlayer(QMainWindow):
    ...
    def load_video(self):
        ...
        filename = 'D:/GitHub/GuitarPlay2Tab/GuitarPlay2Tab/test/guitar4.mp4'
        if filename:
    ...
    def track_objects(self):
        ...
        if len(self.detected_frames) > 1:
            ...
            while track_flag:
                ...
                if not ret:
                    break
                track_flag, fretboard = self.tracker.track(frame, None, self.detected_frames[begin_frame-1][0])
                # self.gui_draw_fretboard(frame, fretboard)
                if track_flag:
                    hand_oriented_bb = (fretboard.oriented_bb[0], 
                                        (fretboard.oriented_bb[1][0]+200, fretboard.oriented_bb[1][1]),
                                        fretboard.oriented_bb[2])
                    ...
                    if results.multi_hand_landmarks:
                        ...
                        for i in range(len(results.multi_hand_landmarks)):
                            hands[i, ...] = transform_points(hands[i, ...], np.linalg.inv(crop_transform)).astype(int)
                        self.detected_frames[begin_frame] = (fretboard, hands)
                    else:
                        self.detected_frames[begin_frame] = (fretboard, None)
                cv2.waitKey(1)
    ...
    def save_data(self):
        ...
        filename = 'D:/GitHub/GuitarPlay2Tab/GuitarPlay2Tab/test/guitar4.pickle'
...
if __name__ == "__main__":
    ...
    player = VideoPlayer(detector=Detector(), tracker=Tracker())
